<?php
// Transformations-Skript als 'im3_transform.php' einbinden
// (Passe den Dateinamen an, falls deine Datei anders heisst, z. B. 'transform.php')
$jsonData = include __DIR__ . '/im3_transform.php';

// Dekodiert die JSON-Daten zu einem Array
$dataArray = json_decode($jsonData, true);

// Fehlerbehandlung: JSON nicht lesbar
if (!is_array($dataArray)) {
    http_response_code(500);
    header('Content-Type: text/plain; charset=utf-8');
    echo "Fehler: Konnte transformierte Daten nicht dekodieren.";
    exit;
}

// Optional: nur die letzten 200 Einträge anzeigen (falls sehr viele Datensätze)
$maxRows = 200;

// Sortieren nach gemessener Zeit (absteigend, neueste zuerst)
usort($dataArray, function ($a, $b) {
    $ta = $a['measured_at_new'] ?? '';
    $tb = $b['measured_at_new'] ?? '';
    // String-Vergleich reicht, da Format 'Y-m-d H:i:s'
    return strcmp($tb, $ta);
});
...
<body>
  <div class="wrap">
    ...
    <p class="meta">
      Einträge: <?= number_format(count($dataArray), 0, ',', '\'') ?> &middot;
      Zeitraum (max. 200, neueste zuerst)
    </p>
